...
const receivedBrand = urlParams.get('brand');
const decodedBrand = decodeURIComponent(receivedBrand);
const brandsPage = window.location.href;
  ...
  function applyFilters() {
    const filteredProducts = products.filter((product) => {
      for (const filter in selectedFilters) {
        if (filter === "brand") {
          if (selectedFilters[filter] !== "" && selectedFilters[filter] !== product.brand) {
            return false; // Skip products with different brands
          }
        } else if (selectedFilters[filter] && selectedFilters[filter] !== product[filter]) {
          return false; // Skip products that don't match other selected filters
        }
      }
  ...
  function renderProducts(productsArray) {
    ...
    productsArray.forEach(product => {
      ...
      div.addEventListener("click", () => {
        window.location.href = `/product_detail.html?id=${id}&brand=${receivedBrand}&brandsPage=${brandsPage}`;
      });

    });

    // Attach event listener to wishlist
...
function generateBreadcrumb() {
  ...
  const brandPage = createBreadcrumbElement(`Brands - "${decodedBrand}"`, "brands.html");
